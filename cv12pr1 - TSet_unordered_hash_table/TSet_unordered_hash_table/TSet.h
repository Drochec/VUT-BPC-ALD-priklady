...
#define TSET_H
/** \file TSet.h
 *  \brief Definice typu Set - (realizace unordered množiny pomocí hašovací tabulky)
 *  \author Petyovský
 *  \version 2024
 *  $Id: TSet.h 2731 2024-04-20 15:14:55Z petyovsky $
 */
...
/** \defgroup TSet 2. Set
 *  \brief Definice datového typu Set a jeho funkcí (realizace množiny pomocí hašovací tabulky)
 *  \{
 */

/** \brief Definice typu Set
 *  \details Typ Set obsahuje hašovací tabulku s přihrádkami elementů typu SetElementHashable a umožňuje s nimi pracovat pomocí definovaného API.
 */
struct TSet
	{
	size_t iSize;						///< Počet elementů množiny
	struct TSetHashTable *iHashTable;	///< PIMPL ukazatel na hašovací tabulku
	};
 ...
 *  \details SetIterator se při vzniku naváže na zvolenou unordered množinu a následně umožňuje číst hodnoty jednotlivých elementů pomocí definovaného API.
 */
struct TSetIterator
	{
