...
#define TQUEUE_H
/** \file TQueue.h
 *  \brief Definice typu fronta (realizace pomocí kruhového pole)
 *  \author Petyovský
 *  \version 2024
 *  $Id: TQueue.h 2615 2024-02-16 15:36:07Z petyovsky $
 */
 ...
 *  \brief Definice datového typu Queue a jeho funkcí (realizace fronty pomocí kruhového pole)
 *  \{
 */

typedef int TQueueElement;					///< Definice typu QueueElement (datový typ elementů fronty)

#define QUEUE_MAXCOUNT 8				///< Maximální počet elementů fronty

/** \brief Definice typu Queue
 *  \details Typ Queue obsahuje pole prvků typu QueueElement a umožňuje s nimi pracovat pomocí definovaného API.
 *  \invariant Obecně platný a testovatelný předpoklad: `(iFrontPos < QUEUE_MAXCOUNT && iBackPos < QUEUE_MAXCOUNT)`.
 */
struct TQueue
	{
 ...
 *  \attention Funkce ověřuje konzistenci obou indexů do pole fronty a pomocí `assert` hlásí (při překladu v režimu `Debug`) případnou běhovou chybu!
 */
