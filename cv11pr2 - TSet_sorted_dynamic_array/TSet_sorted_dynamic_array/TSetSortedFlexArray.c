...
int set_flex_array_search(const struct TSetSortedFlexArray *aFlexArray, TSetElement aValue, size_t *aPosPtr, size_t aSize)
	...
	for (size_t i = 0; i < aSize; i++)
		{
		int cmp = set_element_comparator(&aValue, &aFlexArray->iElement[i]);
		if (cmp <= 0)
			{
			*aPosPtr = i;
			return cmp;
	...
	*aPosPtr = aSize - 1;
	...
	while (min < max)
		{
		size_t s = min + (max - min) / 2;
		int cmp = set_element_comparator(&aValue, &aFlexArray->iElement[s]);
		if (cmp == -1 && s != 0)
			{
			max = s - 1;
			}
	...
	*aPosPtr = min;
	return set_element_comparator(&aValue, &aFlexArray->iElement[min]);
